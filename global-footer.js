...
(function () {
  ...
  function loadCMSData() {
    ...
    document.querySelectorAll('.wizard-model-item .model-data-payload').forEach(item => {
      ...
      if (!indexes.familiesByBrand.has(model.brand)) indexes.familiesByBrand.set(model.brand, new Set());
      indexes.familiesByBrand.get(model.brand).add(model.family);

      const familyKey = `${model.brand}|${model.family}`;
      if (!indexes.variantsByFamily.has(familyKey)) indexes.variantsByFamily.set(familyKey, new Set());
    ...
    document.querySelectorAll('.config-data-item .config-data-payload').forEach(item => {
      ...
      if (!indexes.configsByModel.has(config.modelName)) indexes.configsByModel.set(config.modelName, []);
      indexes.configsByModel.get(config.modelName).push(config);

      if (!indexes.sizesByModel.has(config.modelName)) indexes.sizesByModel.set(config.modelName, new Set());
  ...
  function renderCart() {
    ...
    state.cart.forEach(item => {
      ...
      if (lineTotal !== null) { subtotal += lineTotal; } else { allPriced = false; }
  ...
  function initWizard() {
    ...
    if (contentArea && DOM.cartItems) contentArea.insertBefore(quoteHeader, DOM.cartItems);
  ...
  function goBackFrom(step) {
    if      (step === 'brand')   state.wizState = { brand: null, family: null, variant: null, size: null, filter: null };
    else if (step === 'family')  { state.wizState.family = null; state.wizState.variant = null; state.wizState.size = null; }
    else if (step === 'variant') { state.wizState.variant = null; state.wizState.size = null; }
    renderWizard();
  }
  ...
  function renderWizard() {
    ...
    if (!activeStep) { finishWizard(); return; }
  ...
  function renderActiveStep(step) {
    ...
    switch (step) {
      ...
      case 'family':
        ...
        if (state.wizState.filter?.families) options = options.filter(f => state.wizState.filter.families.includes(f));
      ...
      case 'variant':
        ...
        if (options.length === 0) { state.wizState.variant = ''; renderWizard(); return; }
        if (state.wizState.filter?.variants) options = options.filter(v => state.wizState.filter.variants.includes(v));
        titleText = 'Select Model Type';
        break;
      case 'size':
        const activeModel = data.modelMenu.find(d =>
          d.brand === state.wizState.brand && d.family === state.wizState.family &&
  ...
  function finishWizard() {
    ...
    if (!activeModel) { DOM.activeStep.innerHTML = '<div style="color:#ff5a5f;">Model definition not found.</div>'; return; }
    ...
    if (!config) { DOM.activeStep.innerHTML = '<div style="color:#ff5a5f;">Configuration not found for this size.</div>'; return; }

    const matchingParts = data.parts.filter(p => p.models.includes(activeModel.fullName));
    if (matchingParts.length === 0) { DOM.activeStep.innerHTML = '<div style="color:#888;">No parts found for this model.</div>'; return; }
    ...
    addAllBtn.addEventListener('click', () => addAllPartsToCart(matchingParts, config, machineContextStr));
    ...
    if (visibleCount === 0) listDiv.innerHTML = '<div style="color:#888;text-align:center;padding:10px;">All parts added to order!</div>';
  ...
  function addAllPartsToCart(parts, config, machineContext) {
    ...
    parts.forEach(part => {
      ...
      if (existing) {
        if (existing.qty !== qtyToAdd) { existing.qty = qtyToAdd; existing.machineContext = machineContext; updatedCount++; }
        if (!existing.price && part.price) { existing.price = part.price; updatedCount = updatedCount || 1; }
        if ((!existing.code || existing.code === existing.id) && part.code && part.code !== part.id) existing.code = part.code;
      } else {
        state.cart.push({ id: part.id, name: part.name, code: part.code || part.id, price: part.price || '', image: part.image || '', qty: qtyToAdd, machineContext });
        addedCount++;
      }
    });
    ...
    if (addedCount > 0 || updatedCount > 0) {
      let message = '';
      if      (addedCount > 0 && updatedCount > 0) message = `Added ${addedCount} new item(s) and updated ${updatedCount} item(s) in your cart.`;
      else if (addedCount > 0)                     message = `Added ${addedCount} item(s) to your cart.`;
      else                                         message = `Updated quantities for ${updatedCount} item(s) in your cart.`;
  ...
  function triggerReverseWizard(partName) {
    ...
    if (uniqueFamilies.length > 1) {
      ...
      if (hasRealVariants && uniqueVariants.length > 0) {
        ...
        if (uniqueVariants.length === 1) state.wizState.variant = uniqueVariants[0];
      } else {
  ...
  /* ── Event binding ── */
  function bindEvents() {
    DOM.openBtns.forEach(btn => btn.addEventListener('click', (e) => { e.preventDefault(); showCart(); }));
    ...
    if (addBtn) {
      ...
      if (part) {
        ...
        const machineContext = activeModel
          ...
          : '';
        const wasUpdated = addPartToCart({ name: part.name, id: part.id, code: part.code || '', price: part.price || '', qty, image: part.image, machineContext }, false);
        showUpdateMessage(wasUpdated ? `Updated quantity for ${part.name}` : `${part.name} added to your cart.`);
  ...
  function handleGlobalClick(e) {
    ...
    if (openCartBtn) { e.preventDefault(); showCart(); return; }
    ...
    if (btn.classList.contains('carousel-link')) {
      ...
      if (href === '#open-cart') { e.preventDefault(); showCart(); return; }
      if (href && href !== '#' && href.trim() !== '' && !productName) {
        if (href.startsWith('http') && !href.includes(window.location.hostname)) window.open(href, '_blank');
        return;
      }
      if (productName) { e.preventDefault(); handleAddToQuote(btn, productName); }
    }
  }
  ...
  function handleAddToQuote(btn, productName) {
    ...
    if (canonicalPart) { productId = canonicalPart.id; productName = canonicalPart.name; if (!imageUrl) imageUrl = canonicalPart.image; }

    const part = { id: productId || productName, name: productName, code: btn.getAttribute('data-code') || '', price: btn.getAttribute('data-price') || '', image: imageUrl || '', qty: 1 };
    const existing = state.cart.find(item => item.id === part.id);
...
(function () {
  ...
  function renderQuoteReview() {
    ...
    tableContainer.querySelectorAll('.qr-remove-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        const id      = btn.dataset.id;
        const updated = JSON.parse(localStorage.getItem(CART_KEY) || '[]').filter(i => i.id !== id);
    ...
    if (summaryContainer) {
      ...
      if (subtotal > 0) {
        ...
        `;
        if (!allPriced) summaryHtml += `<div class="qr-summary-note" style="margin-top:8px;">Some items are price on request — our team will confirm final pricing before processing your order.</div>`;
      } else {
    ...
    if (cartDataField) {
      cartDataField.value = cart.map(item => {
  ...
  function initQRForm() {
    ...
    if (business) {
      ...
      invBlock.appendChild(mkInput({ type:'text', id:'qr-invoicing-address', name:'Invoicing Address', placeholder:'Address, Town, State, Postcode' }));
